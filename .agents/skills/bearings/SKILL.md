...
3. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Charted Next** - accepted outcome obligations, queued or gated work, and integrity warnings, including each program's next check or error and each queued item's blocker, date, or integrity reason.
...
4. **Charted Next** - accepted outcome obligations plus queued or gated work waiting on the fleet or a date, and action-free fleet-integrity warnings, never on the captain.
...
- The four buckets are mutually exclusive, so every item is forced into exactly one: needs-your-action is Captain's Call, done is Recently Landed, self-progressing direct-report work is Underway, and accepted outcome obligations, not-yet-started work, or action-free fleet-integrity warnings are Charted Next.
