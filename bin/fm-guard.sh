...
# is active (state/x-watch.check.sh exists), an accepted program is unfinished,
# or a process-event source is registered and supervision is not healthy, prints
...
# source, an accepted unfinished program, or an X-mode relay poll needs supervision.
