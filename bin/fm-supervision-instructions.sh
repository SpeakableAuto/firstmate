...
ordinary_wake_line() {
  printf '%s\n' '- Delivery continuation: after handling a wake or timeout, reconcile due accepted programs and surfaced program errors from bin/fm-programs.sh --json; task completion does not close the product obligation.'
  case "$HARNESS" in
    claude)
