...
if [ "\${1:-}" = --handling-delivered ]; then printf '%s %s\\n' "$2" "$4" >> "$FM_HOME/state/confirmations"; exit 0; fi
printf 'arm\\n' >> "$FM_HOME/state/arms"
count=$(wc -l < "$FM_HOME/state/arms" | tr -d '[:space:]')
printf 'watcher: started pid=%s (beacon fresh) recovery-generation=native-%s\\n' "$$" "$count"
...
`, { mode: 0o755 });
  ...
  async function fire() {
    ...
    const confirmed = rows("confirmations").length;
    ...
    await waitFor(() => rows("arms").length > cycle && rows("confirmations").length > confirmed, "native wake did not restore and confirm");
  }
  ...
  assert.equal(rows("confirmations").length, event, "native bridge lost recovery confirmations");
  assert.deepEqual(errors, []);
  console.log(`ok - Pi ${JSON.parse(readFileSync(`${packageRoot}/package.json`, "utf8")).version} native SDK: 10 busy + 10 handling wakes, steering, abort with restored queue, ${event} durable rows and recovery confirmations`);
} finally {
  releaseAll = true;
