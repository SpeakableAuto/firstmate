...
An owned successor refreshes a retained confirmation when its recovery generation matches.
Because downtime republication preserves a pending generation, a different newly established owned generation proves older retained recovery obligations are superseded so they cannot block confirmation of the current generation.
Repeated delivery after acknowledgement is a no-op only while the generation and live watcher ownership still match.
...
`tests/fm-pi-watch-extension.test.sh` checks Pi's first-cycle-or-explicit-repair tool metadata and ownership-based redundant-call no-ops, then simulates actionable and empty child closes against the actual Pi and OpenCode close handlers, proves the successor launches before a wake offer, same-generation replacement refreshes the retained delivery owner, and a newly established owned generation retires a superseded acknowledged obligation, verifies confirmation waits for a matching transformed identity and single-flight behavior, changes the session lock before close to prove ownership is rechecked, and hangs each successor arm to prove bounded fallback delivery includes the typed restoration failure.
