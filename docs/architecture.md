...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, due accepted-program reconciliation and surfaced program errors, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Routine watcher polling, future accepted-program checks, supervision no-ops, elapsed waiting time, and absorbed benign wakes stay silent.
...
Accepted delivery programs remain native in-flight backlog records rather than worker metadata, a scheduler, or another task database.
The existing watcher derives their due reconciliation from that backlog and delivers it through the ordinary durable check queue, while [`configuration.md`](configuration.md#accepted-delivery-programs) owns the current operator contract and the program helper header owns the exact fields and receipt mechanics.
...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, the complete accepted-program projection, task metadata, current crew state, endpoint probes, PR/report pointers, scout reports, bounded current summaries from registered secondmate homes, and secondmate return-channel guidance.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if worker tasks, accepted programs, process-event sources, or Relay polling have an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when worker tasks, accepted programs, a process-event source, or Relay polling need supervision and no identity-matched watcher lock with a fresh beacon is live, blocking-capable Stop hooks block and nonblocking turn-end integrations force one bounded follow-up.
...
Opt-in private workflow instructions use the indexed `data/workflow/` store and scoped loader described in [`configuration.md`](configuration.md#private-workflow-context-configworkflow-context); they are operating inputs, not project memory or publishable delivery evidence.
