...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call whenever `bin/fm-supervision-lib.sh`'s shared predicate still requires it.
